...
e13_t d2_new_ctx(struct d2_handle* h, char* name){
	...
	if(d2_find_ctx(h, name) == E13_OK) return e13_error(E13_EXISTS);
...
e13_t d2_set_ctx_buf(struct d2_handle* h, char* name, char* buf, size_t bufsize, char flags){
	...
	if(!ctx) {d2_unlock_ctx(h); return e13_error(E13_NOMEM);}

    if(ctx->exps) {d2_unlock_ctx(h); return e13_error(E13_EXISTS);}
...
e13_t d2_rst_ctx(struct d2_handle* h, char* name){
	...
	if(ctx->flags & D2_CTXF_COPY_BUF && ctx->buf) free(ctx->buf);
	if(ctx->exps) free(ctx->exps);
	if(ctx->toks) free(ctx->toks);
...
e13_t d2_run_ctx(struct d2_handle* h, char* name){
	...
	if(!ctx->exps){//not compiled
		...
		} else {d2_unlock_ctx(h); return err;}

		for(ntok = 0; ntok < nexp; ntok++){
			if((err=d2_infix2prefix(exps+ntok)) != E13_OK) {d2_unlock_ctx(h);return err;}
		}        
